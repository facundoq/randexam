...
from .markdown import Document, Enumeration, Paragraphs, Renderable, Sections, Text
...
class QAQuestion(Question):
    ...
    def _generate(self) -> RenderableQA:
        ...
        if self.include_question:
            answers = Enumeration(
                [
                    Paragraphs([qa.q, " RESPUESTA:", qa.a], separator="\n")
                    for qa in self.qas
                ]
            )
        else:
...
class Exam:
    ...
    def generate_exam(
        ...
        intro: Renderable,
        titles: list[str],
        section_bodies: list[Renderable],
        points: list[int],
    ) -> Renderable:
        depth = 3
        ...
        d = Document(
            title, body, subtitle=self.subtitle, date=self.date, geometry=self.geometry
        )
        return d
    ...
    def generate(self, seed: int = None) -> RenderableQA:
        ...
        question_sheet = self.generate_exam(
            self.title, intro_question, titles, questions, points
        )
        if self.questions_in_answers:
            ...
            intro_answers = ""
        answer_sheet = self.generate_exam(
            self.title + " (Respuestas)", intro_answers, titles_answer, answers, points
        )
