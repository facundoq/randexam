...
def parcial(id:int):
    ...
    d = Dataset.from_csv(Path("data/estrellas2023.csv"),class_index=2)
    print(d.attributes)
    
    question_list=[
                questions.quantiles.Quantiles(d,0),
                Boxplot(d,0),
                questions.discretization.Discretization(d,0,["Bajo","Alto"]),
                questions.correlation.CorrelationMatrix(d),
                questions.correlation.CorrelationCoefficient(d,0,1)
